...
class decoder(nn.Module):
    def __init__(self, numberx=192, numbery=96, t=4, activ='tanh'):
        ...
        self.conv5t = nn.Conv2d(self.lc[4], self.lc[3], fs, padding='same')
        self.conv4t = nn.Conv2d(self.lc[3], self.lc[2], fs, padding='same')
        self.conv3t = nn.Conv2d(self.lc[2], self.lc[1], fs, padding='same')
        self.conv2t = nn.Conv2d(self.lc[1], self.lc[0], fs, padding='same')
        self.conv1t = nn.Conv2d(self.lc[0], 1, fs, padding='same')
        
        self.rs = nn.Flatten()
        self.id = nn.Identity()
        
        self.ln1 = nn.LayerNorm([self.lc[0]*2, self.lx[0], self.ly[0]])
        self.ln2 = nn.LayerNorm([self.lc[1]*2, self.lx[1], self.ly[1]])
        self.ln3 = nn.LayerNorm([self.lc[2]*2, self.lx[2], self.ly[2]])
        self.ln4 = nn.LayerNorm([self.lc[3]*2, self.lx[3], self.ly[3]])
        self.ln5 = nn.LayerNorm([self.lc[4]*2, self.lx[4], self.ly[4]])
        self.ln6 = nn.LayerNorm([self.lc[5]*2, self.lx[5], self.ly[5]])
